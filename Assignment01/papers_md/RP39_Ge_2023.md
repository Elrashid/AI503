...
![RP39_Ge_2023 fig01](../figures/RP39_Ge_2023_fig01.jpg)
...
![RP39_Ge_2023 fig02](../figures/RP39_Ge_2023_fig02.jpg)
...
plug-and-play manner. This is a big advantage of FastGen, because the training cost on extra-large models [(Brown et al.,](#page-9-3) [2020)](#page-9-3), can hardly be afforded by many research labs or practitioners.
...
Model profiling is conducted based on the result of prompt encoding. Specifically, for a compression policy C, we mark the corresponding KV cache compression as KC,V^C^ = f(K,V , C), where K^C^ and V^C^ are the compressed KV cache. Then, for attention map A = softmax(QK^T^ ), we pick the optimal policy that can recover A with a recover ratio T with the minimum memory cost:
...
tokens in the KV cache. The length budget of frequent tokens over the current sequence length is controlled by a ratio r_f. This policy is referred to C_{\text{frequent}}.
...
where the sum of two compression strategies is to compute the union of their compressed KV cache, and C_{\rm full} refers to full KV cache without compression.

We use C_{\rm special} as a component in all hybrid policies for two reasons: 1) We observe that high attention scores are usually allocated towards C_{\rm special}, as to be detailed in Section 4, indicating that C_{\rm special} are crucial for attention map recovery; 2) the compressed cache of C_{\rm special} is memory-efficient since there are usually less than 5 special tokens in a sentence. In other words, it brings little-to-no extra memory cost by always including C_{\rm special}. Similarly, C_{\rm punct.} is often used as a component to form hybrid policies due to its memory-efficiency, i.e., the number of punctuations in a sentence is small. The final algorithm for token generation is presented in Algorithm 2.
...
![RP39_Ge_2023 fig03](../figures/RP39_Ge_2023_fig03.jpg)
...
![RP39_Ge_2023 fig04](../figures/RP39_Ge_2023_fig04.jpg)
...
**Backbones.** We conduct experiments with both Llama 1 (Touvron et al., 2023a) and its fine-tuned variants, with model sizes ranging from 7B to 65B. For fined-tuned variants, we do not choose

<!-- page 7 -->

![RP39_Ge_2023 fig05](../figures/RP39_Ge_2023_fig05.jpg)
