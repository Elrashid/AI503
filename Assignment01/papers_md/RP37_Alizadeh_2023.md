...
1. Reducing Data Load: Aiming to decrease latency associated with flash I/O operations by loading less data[1](#page-2-1) .
...
To avoid loading the entire up projection matrix, we follow [Liu et al.](#page-10-3) [(2023b)](#page-10-3), and employ a

> ^1^ It is notable that, by *data* we often refer to the weights of the neural network. However, the techniques we have developed can be easily generalized to other data types transferred and used for LLM inference, such as activations or KV cache, as suggested by [Sheng et al.](#page-11-2) [(2023)](#page-11-2).
...
To further showcase the strength of our method and adaptability to other decoding strategies we have applied speculative decoding on the OPT 6.7B model. The challenge for doing speculative decoding is the limited memory available within DRAM. Given \lambda tokens from the draft model, the big model verifies them and will keep a window of size k for each layer. The model should decide neurons of which tokens to keep in memory before verifications are done. If the model keeps the last k tokens out of \lambda + 1 tokens in memory and most of them get rejected, there will be very few neuron reuse for the next forward pass. We conjecture that if the ratio of the acceptance is \alpha keeping the last ktokens ending with \alpha(\lambda + 1)th token is optimal in DRAM. We used \lambda = 4 and were able to improve the speed of decoding by 1.4x as shown in table 5, this is close to the original 1.58x speedup of speculative decoding.
...
- In Appendix [B,](#page-13-0) we provide additional details on the low-rank predictor introduced in Section [3.](#page-2-0) We evaluate our trained predictors from both accuracy (i.e., their impact on the model's accuracy) and efficiency perspectives (i.e., the additional neurons they predict to be activated).
- Appendix [C](#page-13-1) offers a more detailed description of our experimental setup and implementation for the experiments conducted in Section [4.](#page-5-0)
- In Appendix [D,](#page-18-0) we discuss a negative result regarding the strategy of bundling neurons based on co-activation as a potential method for increasing chunk size (cf. Section [3.2)](#page-3-2). We intentionally include this negative result as we believe it may inspire future research on effective neuron bundling and its utilization for efficient inference.
- In Appendix [E,](#page-18-1) we delve deeper into the review of related works in the literature.
- In Appendix [F,](#page-19-0) we go over implications of llm in flash when going to smaller devices.
- Finally, Appendix [G](#page-19-1) compares the texts generated by the base model with those produced by our models that utilize the predictor.

## B Low-Rank Activation Predictor: Additional Results

## B.1 Sparsity patterns of predictors

The number of neurons predicted to be active will determine the efficiency of our algorithm, the less sparse the predicted activation the more weights will have to be loaded from flash. We evaluated the sparsity patterns over 100 random samples of the C4 validation dataset. In Figure [9](#page-14-0) we can see the sparsity patterns of OPT, Persimmon, and Phi. In OPT, the number of active neurons predicted by the predictor is 3x the amount of actual sparsity observed in the case of dense inference. In Persimmon it is about the same - 3x the required neurons, and in Phi-2 it is roughly 2x the required neurons of the original model that are activated by the predictor. The neurons that are activated by the model and not the predictor are the false negatives. The gap between the neurons active in both the predictor and the model, and the neurons active only in the model is very narrow in all three models, hence

false negatives constitute a small fraction of predictions. To reduce false negatives, the predictor has to "over-predict", which results in loading neurons that are redundant, that is, will have zero activation and no effect on the outcome. An interesting future direction of this work is improving the accuracy of the predictors to be able to load fewer neurons. One observation we had in OPT and Persimmon is the later layers have more active neurons, which can be seen in Figure [9d.](#page-14-0)

## B.2 Accuracy of models using predictors

We evaluate the accuracy of models on public benchmarks with predictors in place. In Table [4](#page-15-0) it can be seen zero shot accuracy of models doesn't drop. Also, we can see that increasing the predictor size for the last 4 layers of Persimmon and Falcon improves the zero-shot metrics. We evaluated models on MMLU [(Hendrycks et al.,](#page-10-12) [2021)](#page-10-12) benchmark as well. We used Instruct Eval's implementaion [(Chia et al.,](#page-9-6) [2023)](#page-9-6) for evaluating MMLU. In Figure [10a](#page-14-1) we can see the MMLU of Persimmon doesn't drop when the last 4 layers use higher rank predictors but this is not the case for lower ranked ones. Phi2's MMLU will drop 2.3 points from the relufied model still keeping at 52 as shown in Figure [10b.](#page-14-1) By increasing the threshold of lowrank predictor we can reduce the amount of data load, this comes with a slight degradation in zeroshot metrics as seen in the Table [4](#page-15-0) for different thresholds of the Persimmon model. We have used threshold=0.7 for Persimmon.
...
DRAM for the Key-Value (KV) cache while primarily focusing on the model size. This strategy is particularly effective when dealing with only one sequence/query at a time.<sup>2</sup>
...
> ^^2^For OPT 6.7 B model with context length 2048 KV-cache requires 2048\times 2d_{\rm model} elements which is only 8% of model size. Also, the KV cache itself can be held in flash memory.
